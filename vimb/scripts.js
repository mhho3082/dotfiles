// Simplyread
// Imported from, and credits to, https://njw.name/simplyread/
// Copyright 2010, 2011 Nick White (under GNU AGPL v3)
if (
  window.content &&
...
function simplyread(nostyle, nolinks) {
  ...
  function count_p(parenttag) {
    ...
    for (var i = 0; i < c.length; i++) {
      if (c[i].tagName == "p" || c[i].tagName == "P") n++;
    }
    return n;
  }

  var doc;
  doc = document.body === undefined ? window.content.document : document;
  ...
  if (nolinks) fresh.innerHTML = fresh.innerHTML.replace(/<\/?a[^>]*>/g, "");
  fresh.innerHTML = fresh.innerHTML.replace(/<\/?span[^>]*>/g, "");
  fresh.innerHTML = fresh.innerHTML.replace(
    /<style[^>]*>/g,
    '<style media="aural">'
  ); /* ensures contents of style tag are ignored */
